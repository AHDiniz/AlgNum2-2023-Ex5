function [x, y, u, er] = bvp2d(a, b, c, d, n, m, k, beta_x_func, beta_y_func, gamma_func, g_func, h_func, q_func, f_func, bound_conditions)
    ...
    a_coeff = @(i) gamma_func(x(idivide(int32(i), n, "fix") + 1), y(mod(i, n) + 1)) + 2 * k * (1 / (h(1) * h(1)) + 1 / (h(2) * h(2)));
    b_coeff = @(i) (-k / (h(1) * h(1))) - (beta_x_func(x(idivide(int32(i), n, "fix") + 1), y(mod(i, n) + 1)) / (2 * h(1)));
    c_coeff = @(i) (-k / (h(1) * h(1))) + (beta_x_func(x(idivide(int32(i), n, "fix") + 1), y(mod(i, n) + 1)) / (2 * h(1)));
    d_coeff = @(i) (-k / (h(2) * h(2))) - (beta_y_func(x(idivide(int32(i), n, "fix") + 1), y(mod(i, n) + 1)) / (2 * h(2)));
    e_coeff = @(i) (-k / (h(2) * h(2))) + (beta_y_func(x(idivide(int32(i), n, "fix") + 1), y(mod(i, n) + 1)) / (2 * h(2)));
    ...
    for i = 1 : numel(bound_conditions)
        ...
        x_index = idivide(int32(bound_condition.condition_index), n, "fix") + 1;
        y_index = mod(bound_condition.condition_index, n) + 1;
